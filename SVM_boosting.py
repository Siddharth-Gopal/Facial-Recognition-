...
def classifier(sample, lagr_multi, support_vector_labels, support_vectors, intercept):
    coef = 1
    power = 2
    def kernel(x1, x2):
        return (np.inner(x1, x2) + coef) ** power
...
def main():
    ...
    clf = SupportVectorMachine(kernel=polynomial_kernel, power=3, coef=1)
    ...
    for i in range(4):
        ...
        lagr_multi, support_vectors_labels, support_vectors, intercept = clf.fit(X, y_train)
        y_pred = []
        classification = []
        ...
        for idx, sample in enumerate(X_train):
            prediction = classifier(sample, lagr_multi, support_vectors_labels, support_vectors, intercept)
...
class SupportVectorMachine(object):
    ...
    def __init__(self, C=1, kernel=rbf_kernel, power=2, gamma=None, coef=4):
        self.C = C
        self.kernel = kernel
        self.power = power
        self.gamma = gamma
    ...
    def adaboost(self, X, y, weights):
        ...
        for idx, sample in enumerate(X):
            prediction = self.classifier(sample)
        ...
        return np.array(y_pred), weights, alpha
